...
<article>
	...
	<footer>
		<Button mode="outline" type="button" on:click={() => dispatch('edit', meetup)}>Edit</Button>
		{#if isLoading}
			<span>Changing...</span>
		{:else}
			<Button color={meetup.isFavorite ? null : 'success'} mode="outline" on:click={toggleFavorite}>
